...
public class CommandHandler {
	...
	private static CodeCompletionMacros eecc;
	...
	public static void loadMacros(String fileName) {
		try {
			...
			if (line != null) {
				String[] args = line.split(seperator);
					if (args[0].equals("addComments")) {
						if (args.length == 2) {
							if (args[1].equals("true")) {
								macroLines.add("addComments__true");
								CodeCompletionMacros.addComments = true;
							} else {
								macroLines.add("addComments__false");
							}
						}
					} else {
						macroLines.add("addComments__false");
				...
				while ((line = reader.readLine()) != null) {
					CodeCompletionMacros.debugMessage += line + "\n";
					macroLines.add(line);
				}
				saveFile();
			}	
		...
		for (int i = 1; i < macroLines.size(); i++) {
	...
	public static void additionHandler(Editor editor, String selection) {
		if (!selection.equals("")) {
			...
			case "removeMacro":
				if (args.length == 2) {
					...
					if (Arrays.stream(keys).anyMatch(key::equals)) {
						...
						CodeCompletionMacros.insertAndSelectText(editor, "Successfully removed macro",
			...
			case "addComments":
				...
				if (args.length == 2) {
					//System.out.println("addComments?: length is 2");
					if (!args[1].equals("true") && !args[1].equals("false")) {
						//System.out.println("addComments?: not true or false: " + args[1]);
						return;
					}
					//System.out.println("addComments?: true or false");
					CodeCompletionMacros.addComments = Boolean.parseBoolean(args[1]);
					macroLines.set(0, "addComments__" + args[1]);
					saveFile();
					//System.out.println("addComments?: changed");
					CodeCompletionMacros.insertAndSelectText(editor, "Changed addComments to " + ((args[1].equals("true")) ? "true" : "false"), editor.getSelectionStart(), editor.getSelectionStop());
				}
				break;
			}
			...
			if (unfinishedMacro.type == "replace") {
				...
				CodeCompletionMacros.insertAndSelectText(editor, "Successfully added macro",
						end - code.length(), end);
				...
				if (unfinishedMacro.carBack == null) {
					...
					CodeCompletionMacros.insertAndSelectText(editor, replacedText,
							start, end);
					...
					CodeCompletionMacros.insertAndSelectText(editor, "Successfully added macro", start, end);
	...
	public static void setCCM(CodeCompletionMacros neweecc) {
		eecc = neweecc;
	}
	...
	public static ArrayList<String> getMacroLines() {
		return macroLines;
	}
