		...
		try {
			...
			if (line != null) {
					...
					if (args[0].equals("addComments")) {
						if (args.length == 2) {
							if (args[1].equals("true")) {
								...
								ExtendedExtendedCodeCompletion.addComments = true;
							} else {
				...
				while ((line = reader.readLine()) != null) {
					ExtendedExtendedCodeCompletion.debugMessage += line + "\n";
					macroLines.add(line);
				}
				saveFile();
			}	
		...
		if (!selection.equals("")) {
			...
			case "addComments":
				...
				if (args.length == 2) {
					...
					ExtendedExtendedCodeCompletion.addComments = Boolean.parseBoolean(args[1]);
					macroLines.set(0, "addComments__" + args[1]);
					saveFile();
					//System.out.println("addComments?: changed");
					ExtendedExtendedCodeCompletion.insertAndSelectText(editor, "Changed addComments to " + ((args[1].equals("true")) ? "true" : "false"), editor.getSelectionStart(), editor.getSelectionStop());
				}
				break;
			}
