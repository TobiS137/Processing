...
public class Input {
	...
	public static Slider createSlider(PApplet pa, int x, int y, int len, int hght, float min, float max, float incr, float start) {
		Slider slider = new Slider(pa, x, y, len, hght, min, max, incr, start);
		inputs.add(slider);
		return slider;
	}

	public static Toggle createToggle(PApplet pa, int x, int y, int scale, boolean startState, int Color) {
